...
def nbr
    result = @journalist.count
    puts "Il y a #{result} handles dans cette liste"
end

def digit
    result = @journalist.count {|x| x[/\d+/]}
    puts "Il y a #{result} handles qui comportent des numéros"
end
...
def first_capital
    sorted = @journalist.map{|x| x.gsub('@', '')}
...
def underscore
    string = @journalist.to_s
    result = string.count('_')
    puts "Il y a #{result} handles qui contiennent un underscore!"
end
def sort 
    result = @journalist.sort {|a,b| a <=> b}
    puts result
end
def short
    result = @journalist.sort_by {|x| x.length}
    puts "VOICI LES 50 PREMIERS AVEC LE MOINS DE CARACTERES Dans le username !".capitalize!
    puts result.first(50)
end
def stalk
    result = @journalist.index("@epenser")  
    puts "La position de @epenser est #{result}!"
end
